...
export function deploymentObservations(d: DeploymentDecision, input?: DeploymentInput): unknown[] {
  return [
    ...
    d.provider_mapping, d.provider_authority_ref, Object.keys(d).filter(k => /adapter/i.test(k)).length,
...
export function buildDeploymentIsolationRegistry(deps: {
  ...
  return [
    ...
    { id: "A22", base: baseInput, note: "toggle default neutrality on (bind provider_authority + PROVIDER_DECISION fact); provider_mapping moves off PROVIDER_NEUTRAL, and provider_authority_ref moves off null in the same bind -- a real, not merely declared, structural pairing since both facts are set at once", mutate: i => { i.provider_authority = { provider: "approved-platform", decision_ref: "provider-decision" }; addFact(i, "PROVIDER_DECISION", "approved-platform", "provider-decision"); } },
    { id: "A23", base: providerInput, note: "rename only the bound decision_ref in place (provider_authority.decision_ref + a new PROVIDER_DECISION fact under that fact_id, same provider name/value); moves provider_authority_ref only -- provider_mapping (A22) is untouched by this probe, proving the two are not mathematically forced to co-vary", mutate: i => { i.provider_authority!.decision_ref = "provider-decision-v2"; addFact(i, "PROVIDER_DECISION", "approved-platform", "provider-decision-v2"); } },
    { id: "A24", base: providerInput, note: "rename only the bound provider name in place (provider_authority.provider + PROVIDER_DECISION fact value, same decision_ref/fact_id), observed for adapter-key absence: the boundary invariant holds (0 adapter keys) even while provider identity legitimately changes; moves provider_mapping (A22) but not provider_authority_ref (A23)", mutate: i => { i.provider_authority!.provider = "different-platform"; i.repository_facts.find(f => f.kind === "PROVIDER_DECISION")!.value = "different-platform"; } },
