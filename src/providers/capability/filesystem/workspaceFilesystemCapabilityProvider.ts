...
import { withTargetWriteLock, writeLockKey } from "./writeSerialization.js";
...
export class WorkspaceFilesystemCapabilityProvider implements CapabilityProvider {
  ...
  private async perform(id: string, path: string, parts: string[], input: Record<string, unknown>, bytes: Buffer | undefined, deadline: Deadline): Promise<Record<string, unknown>> {
    if (id !== "filesystem.write") return this.performOperation(id, path, parts, input, bytes, deadline);
    // Same canonical workspace + same logical target must serialize across every
    // provider instance in this process. The lock is acquired before the target
    // precondition and is held through staging, the final precondition, the
    // publication syscall and temp cleanup; it is released on SUCCESS, BLOCKED,
    // FAIL and TIMEOUT alike. Different targets use different keys and progress
    // independently.
    return withTargetWriteLock(writeLockKey(this.rootStat.dev, this.rootStat.ino, path), () => {
      deadline.check();
      return this.performOperation(id, path, parts, input, bytes, deadline);
    });
  }

  private async performOperation(id: string, path: string, parts: string[], input: Record<string, unknown>, bytes: Buffer | undefined, deadline: Deadline): Promise<Record<string, unknown>> {
