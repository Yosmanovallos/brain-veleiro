...
class Deadline {
  ...
  check(): void { if (performance.now() - this.start >= this.timeout) reject("TIMEOUT"); }
...
export class WorkspaceShellCapabilityProvider implements CapabilityProvider {
  ...
  async invoke(request: ToolInvocationRequest): Promise<ToolInvocationResult> {
    ...
    try {
      ...
      deadline.check();
      const output = await this.execute(profile, cwd, parts, deadline, request.timeout_ms);
  ...
  private message(code: NormalizedToolError["code"]): string {
    switch (code) {
      ...
      case "TIMEOUT": return "The command exceeded the effective execution timeout and its process group was terminated.";
  ...
  private async execute(profile: PreparedProfile, cwd: string, parts: string[], deadline: Deadline, requestTimeoutMs: number): Promise<Record<string, unknown>> {
    const chain = await this.buildChain(parts, deadline);
    try {
      // Final pre-spawn window: containment recheck, then executable identity
      // recheck, then a synchronous deadline check, then the immediate spawn.
      // No unrelated await is permitted between the identity recheck and spawn.
      ...
      const outcome = await runProcessGroup(child, {
        ...
        timeoutMs: Math.max(1, Math.min(requestTimeoutMs, profile.maxTimeoutMs)),
