...
import { withShellSandbox, profile, create, config, fixtureSource } from "./fixtures.js";
...
it("overflow fixtures gate flooding on grandchild-written marker readiness", () => {
  for (const name of ["treeflood", "stubbornflood"] as const) {
    const source = fixtureSource(name);
    const markerWrite = source.indexOf("f.writeFileSync(process.argv[1]");
    const readySend = source.indexOf("process.send({ event:'ready'");
    const readyHandler = source.indexOf('gc.once("message"');
    const markerCheck = source.indexOf('f.readFileSync(marker, "utf8")', readyHandler);
    const proofWrite = source.indexOf('f.writeFileSync(marker + ".ready"', markerCheck);
    const flood = source.indexOf("setInterval(() => process.stdout.write(b), 5)");
    ...
    expect(readySend).toBeGreaterThan(markerWrite);
    expect(readyHandler).toBeGreaterThan(readySend);
    expect(markerCheck).toBeGreaterThan(readyHandler);
    expect(proofWrite).toBeGreaterThan(markerCheck);
    expect(flood).toBeGreaterThan(proofWrite);
    const handler = source.indexOf("process.on('SIGTERM', () => {})");
    if (name === "stubbornflood") {
      expect(handler).toBeGreaterThanOrEqual(0);
      expect(handler).toBeLessThan(markerWrite);
    } else {
