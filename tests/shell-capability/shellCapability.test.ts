...
import { assertBoundaries, closureClaims, finalSpawnGap, inferredScope, phaseText, productionSources, shellFutureSurface, startGroupBodyAwaits, text, unrelatedAwaits } from "./audit.js";
import {
  boundedEvidence, configTimeoutBounds, executableDrift, explicitRootOnly,
  externalSignal, permissionDeniedAtSpawn, stdioShape,
...
const invariants: Record<string, () => void | Promise<void>> = {
  ...
  "S14C-HI-021": all(positives["FX-POS-008"], configTimeoutBounds),
  ...
  "S14C-HI-026": negatives["FX-NEG-038"],
  "S14C-HI-027": all(positives["FX-POS-003"], externalSignal),
  "S14C-HI-028": all(positives["FX-POS-013"], async () => {
  ...
  "S14C-HI-029": negatives["FX-NEG-039"],
...
for (const [label, cases] of [["canonical positives", positives], ["canonical negatives", negatives], ["canonical hard invariants", invariants]] as const) {
  describe(label, () => { for (const [id, exercise] of Object.entries(cases)) it(id, exercise as () => Promise<void>, 60000); });
}
