...
process.stdout.write("parent " + process.pid + " gc " + gc.pid); setInterval(() => {}, 1e9);`,
  stubbornflood: `const cp = require("node:child_process"); const marker = process.argv[2];
cp.spawn(process.execPath, ["-e", "process.on('SIGTERM', () => {}); require('fs').writeFileSync(process.argv[1], String(process.pid)); setInterval(() => {}, 1e9)", marker], { stdio: "ignore" });
const b = Buffer.alloc(65536, 120); setInterval(() => process.stdout.write(b), 5);`,
};
