...
// S14B-CONC-HI-010: nothing but the publication syscall may run between the final
// pre-commit precondition and `fs.link` / `fs.rename` in the production writer.
export function finalPublicationGap(): string {
  const src = text("src/providers/capability/filesystem/workspaceFilesystemCapabilityProvider.ts");
  const start = src.lastIndexOf("await this.precondition(target, input, deadline)");
  const link = src.indexOf("await fs.link(", start);
  const rename = src.indexOf("await fs.rename(", start);
  const end = Math.min(...[link, rename].filter(i => i >= 0));
  if (start < 0 || !Number.isFinite(end)) throw new Error("final publication window markers not found");
  return src.slice(start + "await this.precondition(target, input, deadline)".length, end);
}
export const unrelatedAwaits = (gap: string) => (gap.match(/\bawait\b/g) ?? []).length;

// S14B-CONC-HI-011: the builder report must disclose the final-syscall residual
// and must not assert the stronger properties the clarification forbids.
export const reportPath = "brain-bootstrap/reports/S14B-filesystem-capability-verification.md";
export const reportText = () => text(reportPath);
export const affirmativeAtomicClaims = (t: string) =>
  (t.match(/\b(?:provides?|guarantees?|ensures?|offers?|delivers?|is|are)\s+(?:an?\s+|full\s+|true\s+)?(?:atomic compare-and-swap|race-free(?:\s+overwrite)?\s+against\s+all\s+(?:host\s+)?processes|absolute hostile-topology containment)/gi) ?? []).length;
