...
export function synthesize(input: DeploymentInput, prose: string): DeploymentDecision {
  ...
  const blocked: DeploymentDecision = { status: "BLOCKED", reason_code: "BLOCKED_NO_DEPLOYABLE_ENTRYPOINT", deployment_identity: null, entrypoint_assessment: null, build_plan: null, container_plan: null, environment_plan: [], health_plan: null, persistence_plan: null, provider_mapping: "PROVIDER_NEUTRAL", provider_authority_ref: null, deployment_verification: { accepted_evidence_ids: [], missing_kinds: [] }, blockers: ["BLOCKED_NO_DEPLOYABLE_ENTRYPOINT"], limitations: ["No deployment execution or independent verification is performed."], residual_unknowns: [], evidence_refs: [] };
  ...
  return { status: partial ? "PARTIAL" : "READY", reason_code: partial ? "DEPLOYMENT_EVIDENCE_INSUFFICIENT" : null,
    ...
    provider_mapping: method.provider ? i.provider_authority?.provider ?? "PROVIDER_NEUTRAL" : "PROVIDER_NEUTRAL", provider_authority_ref: method.provider ? i.provider_authority?.decision_ref ?? null : null, deployment_verification: method.evidence ? { accepted_evidence_ids: accepted.map(e => e.evidence_id).sort(), missing_kinds: missing } : { accepted_evidence_ids: [], missing_kinds: [] }, blockers: [], limitations: blocked.limitations, residual_unknowns: missing, evidence_refs: [...new Set(i.repository_facts.map(f => f.fact_id))].sort(),
  };
}
