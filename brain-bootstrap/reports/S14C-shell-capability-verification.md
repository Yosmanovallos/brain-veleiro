# S14C Shell Capability — Round 4 Builder Verification

Status: S14C ROUND4 BUILDER PASS AWAITING CONTROL-PLANE SOURCE AUDIT / INDEPENDENT VERIFICATION
S14C: ROUND4_BUILDER_PASS_AWAITING_CONTROL_PLANE_SOURCE_AUDIT
S14: IN_PROGRESS / NOT_CLOSED
HI-054: NOT_AWARDED
S14D: NOT_AUTHORIZED

This report is factual builder evidence for the exact Round-4 S14C candidate on
branch `s14c-shell-capability-part-b-round4`. It does not claim independent-verifier
...
All three prior source-audit FAILs are preserved, not erased. Round-1 `25068c…`,
Round-2 `774f2bf…` and Round-3 `acc7785…` remain on `origin`, unmoved.
`git merge-base --is-ancestor <rejected> HEAD` = **false** for all three, and the
merge-base of each rejected candidate with the Round-4 candidate is exactly
`main` `3cd344d018dbf2d40a39a907a3494bba8f3d940b`. Round 4 was built in a fresh
WSL-native clone (`/home/yosman/brain-s14c-round4`), branched directly from that
exact `main`; no rejected builder worktree was used as the authoritative Round-4
surface.
...
- Rejected branches present and unmoved on `origin`:
  ...
  `s14c-shell-capability-part-b-round3`
  = `acc7785933743048ebb75e4284b127e97ee3b237` (`acc7785^` = `3cd344d…`).
- Round-4 branch `s14c-shell-capability-part-b-round4` cut from exact `main` in a
  fresh isolated WSL-native clone (`/home/yosman/brain-s14c-round4`, POSIX/LF).
  No rejected builder worktree was reused as the authoritative build surface.
...
erratum was authored; Round 4 satisfies the existing contract as written,
including §26.

## 6. Exact candidate scope against baseline `3cd344d…`

`git diff --name-status 3cd344d.. HEAD`: **13 additions + exactly two authorized
modifications**. The S14C additions are the same file set as Rounds 2–3 (no new
files; no `src/providers/capability/shell/**` change vs Round-3 `acc7785…`). The
only new bytes in Round 4 are in `tests/shell-capability/audit.ts`
(second-`ALLOWED_MODIFIED` entry + `concurrencyExercisesMaintenanceExact`),
`tests/shell-capability/regressions.test.ts` (one added `it()`), this report, and
the two `M` files.
...
`execution.ts`, `workspaceShellCapabilityProvider.ts` — **byte-identical to Round-3
`acc7785…`**. `node:child_process` only; no new dependency.
...
## 10. Round-4 QA sequence and results (Node v24.19.0 / npm 11.17.0)
...
- Candidate on branch `s14c-shell-capability-part-b-round4`, a strictly linear
  ...
  force push. Rejected branches `s14c-shell-capability-part-b` (`25068c…`),
  `s14c-shell-capability-part-b-round2` (`774f2bf…`) and
  `s14c-shell-capability-part-b-round3` (`acc7785…`) are not moved. No merge to
  `main`; no `main` movement; no `STATE.yaml` / `CURRENT.md` phase-closure edit.
  The exact remote Round-4 candidate SHA to source-audit is stated in the
  `S14C_ROUND4_BUILDER_HANDOFF` relay for this gate.

## 14. Next action

CONTROL_PLANE_SOURCE_AUDIT_OF_THE_EXACT_ROUND4_REMOTE_CANDIDATE, then a fresh
